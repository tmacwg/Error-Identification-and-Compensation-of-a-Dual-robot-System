...
%读取标志点,并计算每组标志点的旋转平移矩阵B
 A0=load('E:\重庆大学研究\论文撰写\双机器人系统标定和误差补偿\实验图片\蒋诚华科实验\A_Mat.txt');
 B0=load('E:\重庆大学研究\论文撰写\双机器人系统标定和误差补偿\实验图片\蒋诚华科实验\B_Mat.txt');
 C0=load('E:\重庆大学研究\论文撰写\双机器人系统标定和误差补偿\实验图片\蒋诚华科实验\C_Mat.txt');
 [a,~]=size(A0);
 for i=1:a
     A((4*i-3):4*i,:)=[A0(i,1:4)',A0(i,5:8)',A0(i,9:12)',A0(i,13:16)'];
     B((4*i-3):4*i,:)=[B0(i,1:4)',B0(i,5:8)',B0(i,9:12)',B0(i,13:16)'];
     C((4*i-3):4*i,:)=[C0(i,1:4)',C0(i,5:8)',C0(i,9:12)',C0(i,13:16)'];
 end
